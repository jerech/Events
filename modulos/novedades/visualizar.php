			...
			<div id="divPrint" class="ibox-content p-xl">
				<div class="row">
					...
					<div class="col-sm-6">
						...
						<?php
						$date = date_create($oNovedad->getCreated_at());
						$created_at = date_format($date, 'd/m/Y H:m:s');

						$date = date_create($oNovedad->getUpdated_at());
				...
				<div class="row">
					...
					<div class="col-sm-10">
						<textarea class="form-control" readonly style="resize:none;"><?=$oNovedad->getObservacion();?></textarea>
					</div>
				</div>
			</div>
			<br>
			<div class="text-right">
				<button class="btn btn-primary" onclick="window.location.href = '<?php echo BASE_URL ?>novedades/&m=<?php echo $_REQUEST["m"] ?>'">Regresar</button>
			</div>
		</div>
	</div>
</div>