...
unset($aWhere);
            ...
            else
            {
			 ...
			 $aAsignaciones=$objAsignacion->buscar_frecuencia($aWhere,$aOrWhere,"","","u.nombre || '_' || p.nombre","ASC","a.usuario_numero,u.nombre,p.nombre,p_dest.nombre","a.usuario_numero,u.nombre,p.nombre as punto,p_dest.nombre as punto_dest,count(*) as totalAsignaciones");	
...
$objPHPExcel->getActiveSheet()
	...
	->setCellValue('B7', 'PUNTO CHEQUEO ORIGEN')
	->setCellValue('C7', 'PUNTO CHEQUEO DESTINO')
	->setCellValue('D7', 'TOTAL ASIGNACIONES')
	;
	...
	if (count($aAsignaciones)>0)
	{
		foreach($aAsignaciones as $objeto)
		{
		       $objPHPExcel->getActiveSheet()
					...
					->setCellValue('B'.$row, $objeto->punto)
					->setCellValue('C'.$row, $objeto->punto_dest)
					->setCellValue('D'.$row, $objeto->totalasignaciones);
...
header('Content-Disposition: attachment;filename="REPORTE_FRECUENCIA_'.date('Y-m-d').'.xlsx"');
