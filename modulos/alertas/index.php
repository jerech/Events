<?php
	...
	switch(strtoupper($accion))
	{
		case 'VISUALIZAR':
			...
			$oAlerta=New Alertas($_REQUEST['id']);
			$oAlerta->pasarAleido();
			$oAsignacion=New Asignaciones($oAlerta->getIdasignacion());
			unset($aWhere);
		...
		default:
			...
			if (isset($_POST['buscar']) && !empty($_POST['buscar'])) {
				...
				if (!empty(trim($_POST['filtro_fecha_search']))) { 
					...
					$aWhere["to_char(a.created_at,'YYYY-mm-dd')"] = $y."-".$m."-".$d; 
				}
			}

			$aAlertas=$objAlertas->buscar($aWhere);
